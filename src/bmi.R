...
count_by_ageg<-c(
  sapply(list(18:24,25:44,45:64,65:74),function(r){sum(VarstaRO2010[VarstaRO2010$Varsta %in% r,'TotalM'])}),
  sapply(list(18:24,25:44,45:64,65:74),function(r){sum(VarstaRO2010[VarstaRO2010$Varsta %in% r,'TotalF'])}))
...
stripplot(BMI~ageg|Tip,data=IU,
  ...
  prepanel = function(x, y, ...) { x<-list(ylim = rev(range(IU$BMI)));print(x);x },
...
combined<-rbind(cbind(EHIS_OB,data.frame(study='EHIS')),cbind(bmig.counts.ehis,data.frame(study='EIU')))
