...
likertScale.7<-c("Încîntat","Mulțumit","Oarecum satisfăcut",
...
IU.FEFMP.LIKERT <-as.data.frame(xtabs( ~ factor( value,1:5,c('Nimic','Sesizabil','Mediu','Satisf?c?tor','Puternic') )+Trt,IU.FEFMP))
...
local({
  ...
  g.res.uss_trt.hist<-styleGraph(
    histogram(~value|Trt,IU.USS,nint=7,type="percent",
              ...
              main="USS")
    ,ylab="Procent",xlab="Num?rul de vizite"
  )
